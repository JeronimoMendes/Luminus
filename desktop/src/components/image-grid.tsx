...
function groupFrameRanges(frames: number[], gap = 2): { start: number; end: number }[] {
...
function groupByDate(items: MediaItem[]): { label: string; date: string; items: MediaItem[] }[] {
  const groups = new Map<string, MediaItem[]>();

  for (const item of items) {
    const dateKey = item.datetime ? item.datetime.split(/[T ]/)[0] || "Unknown" : "Unknown";
...
function MediaCard({ item, onClick }: { item: MediaItem; onClick: () => void }) {
  ...
  if (item.type === "video") {
    ...
    return (
      ...
      >
        {thumbSrc ? (
          <img src={thumbSrc} alt={item.filename} className="h-full w-full object-cover" loading="lazy" />
        ) : (
...
function VideoModalPlayer({ selected }: { selected: MediaItem & { type: "video" } }) {
  ...
  return (
    <div className="flex flex-col items-center" onClick={(e) => e.stopPropagation()} onMouseMove={resetHideTimer}>
      <motion.video
        ref={videoRef}
        autoPlay
        preload="auto"
      ...
      >
        <button type="button" onClick={togglePlay} className="text-white hover:text-white/80 shrink-0">
        ...
        >
          ...
          {hasFrames &&
            groupFrameRanges(frames).map((range) => {
              ...
              const widthPct = Math.max(0.5, ((range.end - range.start) / duration) * 100);
              return (
                <button
                  key={range.start}
                  type="button"
...
export function ImageGrid({ images, selected, onSelect, onClose, preserveSearchOrder = false }: ImageGridProps) {
  const groups = useMemo(() => (preserveSearchOrder ? [] : groupByDate(images)), [images, preserveSearchOrder]);
          ...
          {preserveSearchOrder ? (
            <div className="px-8 pb-8">
              <div className="flex flex-wrap gap-2">
                {images.map((item) => (
                  <MediaCard key={item.path} item={item} onClick={() => onSelect(item)} />
                ))}
              </div>
            </div>
          ) : (
            <div className="px-8 pb-8 space-y-6">
              {groups.map((group) => (
                <div key={group.date}>
                  <h3 className="text-[13px] font-semibold text-muted-foreground mb-3">{group.label}</h3>
                  <div className="flex flex-wrap gap-2">
        ...
        {selected && (
          ...
          >
            ...
            >
              ...
              {selected.type === "image" && (selected.camera_model || selected.lens_model) && (
                <p className="text-xs text-white/60">
                  {[selected.camera_model, selected.lens_model].filter(Boolean).join(" · ")}
                </p>
              )}
              {selected.type === "image" && (selected.aperture || selected.iso || selected.exposure) && (
                <p className="text-xs text-white/60">
                  {[
