...
export function loadSettings(): SearchSettings {
  try {
    ...
    if (stored) {
      ...
      return {
        distanceThreshold: Number.isFinite(dist) && dist > 0 ? dist : defaultSettings.distanceThreshold,
        queryLimit: Number.isInteger(limit) && limit >= 1 ? limit : defaultSettings.queryLimit,
      };
    }
  } catch {}
